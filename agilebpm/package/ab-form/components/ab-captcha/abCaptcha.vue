    ...
    if (
      ...
      if (regression.value) {
        if (index > viewValue.value) {
          const title = props.param[viewValue.value + 1]?.label
          ElMessageBox.confirm(
            `是否进入下个进度【${title}】？ 进入下个进度后无法回退！`,
            '提示',
            {
            ...
            viewValue.value = props.param[viewValue.value + 1]?.value
          })
        }
      } else {
