    ...
    >
      ...
      >
        ...
        >
          ...
          <el-form-item label="手机号取值" prop="phoneInfo.value" v-if="info.selectValue.indexOf('sms')!=-1"  :rules="[{ required: true, message: '请选择手机号取值字段' }]">
            <bo-variable-select-tree
              v-model="data.nodeData.phoneInfo"
              data-type="all"
              :default-expand-all="false"
              placeholder="请选择"
            />
          </el-form-item>
          <el-form-item label="邮箱取值" prop="emailInfo.value" v-if="info.selectValue.indexOf('email')!=-1"  :rules="[{ required: true, message: '请选择邮箱地址取值字段' }]">
            <bo-variable-select-tree
              v-model="data.nodeData.emailInfo"
              data-type="all"
              :default-expand-all="false"
              placeholder="请选择"
            />
          ...
          <el-form-item label="邮箱附件取值" prop="emailFile.value" v-if="info.selectValue.indexOf('email')!=-1"  >
            <bo-variable-select-tree
...
<script lang="ts" setup>
  ...
  const props = defineProps({
    pluginData: {
      required: true,
      type: Array as PropType<Array<ExternalNodeMessage>>,
    },
  ...
  const formRef = ref()
  const data = reactive({
    nodeData: {} as ExternalNodeMessage,
    eventIsDis: false,
  })
  ...
  const info = reactive({
    selectValue: [] as any,
    msgTypeList: [{'key':'email','text':'邮箱'},{'key':'sms','text':'短信'}],
  })
  const deletePlugin = () => {
    ElMessageBox.confirm('确定删除此插件吗?', '提示', {
      ...
      .then(() => {
        delete flowSetting.value.plugins['externalNodeMessage']
      })
      .catch(() => {})
  }
  ...
  const addNodeDialog = () => {
    ...
    info.selectValue = ['email']
  }
  const editNodeDialog = (row: ExternalNodeMessage, indexNum: number) => {
    dialogVisible.value = true
    data.nodeData = abUtil.clone(row)
    info.selectValue = data.nodeData.msgType.split(',')
    index = indexNum
  }
...
watch(
  ...
  (newValue: any) => {
    if (newValue) {
      data.nodeData.msgType= newValue.join(',')
    }
  }
)
