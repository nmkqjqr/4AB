<template>
  <div v-if="data.dataSelectorConfig">
    <label class="el-form-item__label" style="padding: 0">岗位选择配置：</label>
...
<script lang="ts" setup>
 ...
 const initData = ()=>{
    ...
    data.value.dataSelectorConfig.dialogKey = 'postSelector'
    data.value.dataSelectorConfig.selectType = 'post'
    data.value.dataSelectorConfig.valueField = 'id'
    data.value.dataSelectorConfig.labelField = 'name'
    data.value.dataSelectorConfig.param = 'id'
  ...
  initData()
   bizApi.customDialog.getDialogByCode("postSelector").then(
    (result: any) => {
      pageData.dialogInfo = result.data
       getParam(result.data.returnFields)
    },
    () => {
      ElMessage.error('获取岗位对话框失败')
    }
  )
